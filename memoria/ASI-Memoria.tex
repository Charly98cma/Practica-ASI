...
El servidor del servicio \textit{NIS} se ha implementado siguiendo los pasos descritos por el profesorado, que son los siguientes:\\ 
...
Tras esto, escribrimos el nombre del dominio proporcionado por el fichero de configuracion en el fichero \textit{/etc/defaultdomain}. Ademas, tras este paso, reiniciamos el servicio utilizando el comando \textit{service nis restart}.\\

En siguiente lugar, ejecutamos el comando \textit{/usr/lib/yp/ypinit -m} para actualizar la base de datos del servicio \textit{NIS} que previamente hemos reiniciado.\\

Y por ultimo, reiniciamos el servicio \textit{NIS} una vez mas, utilizando el mismo comando mencionado anteriormente.\\


\subsubsection{Cliente}
Al igual que el servidor, el cliente del servicio \textit{NIS} se ha implementado siguiendo los pasos definidos por el material proporcionado por el profesorado, a continuacion se encuentran dichos pasos:\\
...
A continuacion escribimos los datos del servidor utilizando el formato \textit{domain \$DOMAIN\_NAME server \$SERVER\_ADDR} en el fichero \textit{/etc/yp.conf}.\\

Tras esto, configuramos el fichero /etc/nsswitch.conf, donde añadimos la palabra \textit{nis} a las variables \textit{passwd, group, shadow} y \textit{hosts}.

Y por ultimo, reiniciamos el servicio \textit{NIS} utilizando el mismo comando detallado en el servidor (\textit{service nis restart}).\\
...
\begin{itemize}
  ...
  \item \textbf{41}: Error al configurar la variable MERGE_PASSWD en el fichero /var/yp/Makefile
  \item \textbf{42}: Error al configurar la variable MERGE_GROUP en el fichero /var/yp/Makefile
  \item \textbf{43}: Error al añadir el nombre del dominio al fichero /etc/defaultdomain
  \item \textbf{44}: Error al actualizar la base de datos del servicio NIS
  \item \textbf{45}: Error al reiniciar el servicio NIS.
\end{itemize}
\subsubsection{Cliente}
